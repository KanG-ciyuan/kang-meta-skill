    ...
    def test_evidence_report_version_mismatch_fails(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (root / "reports" / "skill-ir.json").write_text(
                json.dumps({"package": {"name": "kang-test", "version": "1.0.0"}}), encoding="utf-8"
            )
            ...
            (root / "reports" / "creation-handoff.md").write_text("kang-test 2.0.0", encoding="utf-8")
            failures: list[str] = []
            warnings: list[str] = []

            VALIDATE_SKILL.validate_evidence_reports(
                root,
                {"name": "kang-test", "version": "2.0.0", "maturity_tier": "governed"},
                failures,
                warnings,
            )
