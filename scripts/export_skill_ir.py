#!/usr/bin/env python3
"""Export a compact Skill IR document from a Kang skill package."""
...
SCHEMA_VERSION = "2.0.0-kang-lite"
...
def build_ir(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "intent": {
            ...
            "target_users": intent.get("target_users", ["Kang operator"]),
            "inputs": intent.get("inputs", []),
            "outputs": intent.get("outputs", []),
            "exclusions": intent.get("exclusions", []),
            "kang_defaults": manifest.get("kang_defaults", {}),
        },
        "triggers": {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export compact Skill IR for a Kang skill.")
