#!/usr/bin/env python3
"""Validate the lightweight Kang skill package contract."""
...
def validate(root: Path) -> dict[str, Any]:
    ...
    if root.name == "kang-meta-skill":
        ...
        for relative in ("scripts/research_prior_art.py", "scripts/release_check.py", "scripts/publish_skill.py"):
            if not (root / relative).is_file():
                failures.append(f"kang-meta-skill missing built-in factory script: {relative}")
    ...
    if skill_md.exists():
        ...
        if frontmatter.get("name") == "kang-meta-skill":
            for token in ("skill", "kang", "workflow"):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate a Kang skill package.")
